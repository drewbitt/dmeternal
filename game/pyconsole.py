...
class Console:
    def __init__(self, screen, rect, campaign, functions={}, key_calls={}, vars={}, syntax={}): #party
        ...
        #Added self.camp.party holder variable
        self.party = campaign.party
        # ------------------------------------
    ...
    def send_pyconsole(self, text): # Added party reference here (now removed, testing changes)
        '''\
        ...
        try:
            ...
            # Statement in the form $name = value	
                ...
                # Function
                out = self.func_calls[tokens[0]](*tokens[1:]) #HERE IS WHERE FUNCTIONS ARE BEING CALLED
    ...
    def process_input(self): # Added party reference as parameter
        '''\
                ...
                ## Normal character printing
                    ...
                    if mods & KMOD_CTRL:
                        if event.key in range(256) and chr(event.key) in self.key_calls:
                            self.key_calls[chr(event.key)]() # Added party here for dev commands needing to access player's party
                    else:
